...
- (void)viewDidLoad {
    ...
    self.volumeSlide.minimumValue = 0.0f;
    self.volumeSlide.maximumValue = 10.0f;
    self.volumeSlide.value        = 5.0f;
    
    [self.volumeSlide addTarget:self action:@selector(volumeChange) forControlEvents:UIControlEventValueChanged];
...
- (void)volumeChange
{
    [YYPlayTool sharedYYPlayTool].player.volume = self.volumeSlide.value;
}
