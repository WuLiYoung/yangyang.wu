...
namespace UserInterface
{
    public partial class frmTestState : WeifenLuo.WinFormsUI.Docking.DockContent
    {
        ...
        public void ShowTestFinish(int index,int result)
        {
            ...
            lblTitle[index].Text = string.Format("{1}", index, "UUT"+(index+1)+strResult);
        ...
        private bool IsTesting(int index)
        {
            if (index<0)
            {
                for (int i = 0; i < 6;i++ )
                {
                    ...
                    if (bEnable)
                    {
                        if (GT_UserInterface.m_TestEngine.IsTesting(i)>0)
                        {
                            return true;
                        }
                    }
            ...
            {
                return GT_UserInterface.m_TestEngine.IsTesting(index)>0;
            }
        }
        ...
        private void btnUnitStart_Click(object sender, EventArgs e)
        {
            ...
            if (IsTesting(-1))
            {
                MessageBox.Show("TestEngine is running,please stop it firstly!");
                return;
            }
            if (NeedScanSN())
            {
        ...
        {
            Label[] lblTitle = { lblStatus0, lblStatus1, lblStatus2, lblStatus3, lblStatus4, lblStatus5 };
            int index = Convert.ToInt32((sender as CheckBox).Tag);

            bool bChecked = (sender as CheckBox).Checked;

            lblTitle[index].Enabled = bChecked;
            lblTitle[index].BackColor = bChecked?Color.Cyan:Color.Gray;

            string key = string.Format("{0}{1}", tmMarcos.kUUTEnable, index);
            TestContext.m_dicConfig[key] = bChecked;
            FixtureController.UUT_READY_UP[index] = bChecked;
            FixtureController.UUT_READ_UP_COPY[index] = bChecked;
        }

        public void SetSN(string sn, int index)
        {
            Label[] lblTitle = { lblStatus0, lblStatus1, lblStatus2, lblStatus3, lblStatus4, lblStatus5 };
            lblTitle[index].Text = sn;
        }
    }
}
