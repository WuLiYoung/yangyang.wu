    ...
    public partial class TestUnit : WeifenLuo.WinFormsUI.Docking.DockContent
    {
       ...
       DataLogger.DataLogger testflow = new DataLogger.DataLogger();
        ...
        int OnTestStop(object sender, object arg)
        {
            ...
            (this.MdiParent as MainForm).m_frmState.ShowTestFinish(id, state);
            return 0;
        }
        ...
        int OnItemFinish(object sender, object arg)
        {
           ...
           // id = 0;fail
            if (state==0)
            {
                ...
                lvTestItem.Items[index].SubItems[5+ id].Text = val;
                lvTestItem.Items[index].SubItems[5 + id].ForeColor = Color.Red;
                if(id==0)
                {
                    string tempcode;
            ...
            else if (state==1)
            {
                ...
                testflow.push_testflowlog(messres);
                lvTestItem.Items[index].SubItems[5 + id].Text = val;
                lvTestItem.Items[index].SubItems[5 + id].ForeColor = Color.Green;

            }
            else if(state==2)
            {
                lvTestItem.Items[index].SubItems[5 + id].Text = "Skipped";
                lvTestItem.Items[index].SubItems[5 +id].ForeColor = Color.Cyan;
            }
        ...
        int OnTestFinish(object sender, object arg)
        {
         ...
         //   tc.m_dicContext[tmMarcos.kContextMLBSN] = null;
// delete SN buffer
/*
             for (int i = 0; i < 6;i++ )
             {
...
//
       ...
       private void lvTestItem_SelectedIndexChanged(object sender, EventArgs e)
       {

       }

       private void tbUnitSN_TextChanged(object sender, EventArgs e)
       {
