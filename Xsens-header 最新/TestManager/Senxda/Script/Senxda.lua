...
function M.SetDutIndex(index)
    return tonumber(EEBroad:SetDut(tonumber(index)))
end
...
function M.HoldAirCylinderPositionStatus(index,lock)
	if 	(tonumber(index) == 0) then
		if (tostring(lock) == "lock") then
			M.WriteString("SEN_HOLD_OUT_A\r\n")
			Delay(200)
			return tostring(M.ReadString())
		elseif (tostring(lock) == "unlock") then
			M.WriteString("SEN_HOLD_IN_A\r\n")
			Delay(200)
			return tostring(M.ReadString())
		end
	elseif (tonumber(index) == 1) then
...
function M.CheckRaster(index)
	...
	if (tonumber(index) == 0) then
		...
		DbgOut(str)
        if (string.match(str,"OPEN") ~= nil) then
			return 0
		elseif (string.match(str,"CLOSE") ~= nil) then
			return 1
		else
			return -1
		end
...
function M.CheckSafeSwitch()
    ...
    if (string.match(str,"safe_switch_Open") ~= nil) then
		return 0
	elseif (string.match(str,"safe_switch_Close") ~= nil) then
		return 1
	else
		return -1
	end
