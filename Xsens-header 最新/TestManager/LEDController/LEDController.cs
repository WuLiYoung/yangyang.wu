...
{
    public class LEDController:SerialPort
    {
        ...
        public void Init()
        {
            ...
            try
            {
                if (GT_LEDController.mConfigInfo.ContainsKey(tmMarcos.kLEDConfig))
                {
                    string config = GT_LEDController.mConfigInfo[tmMarcos.kLEDConfig].ToString();
                    this.Setting = config;
                }
                else
                {
                    this.Setting = "115200,N,8,1";
                    GT_LEDController.mConfigInfo[tmMarcos.kLEDConfig] = this.Setting;
                }
                if (GT_LEDController.mConfigInfo[tmMarcos.kLEDCom] != null)
                    this.PortName = GT_LEDController.mConfigInfo[tmMarcos.kLEDCom].ToString();
            }
            catch (Exception)
            {
            }
        ...
        public void DoConnect()
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show("ALSC-1 打开失败 " + e.Message + "(E0002)");
            }
        }
        ...
        private void Get_AlSC_Data()
        {
            ...
            {
                NotificationCenter.DefaultCenter().Notification2Log(0, "Pulsar 1 :Read Left Pulsar Calibration Value fail!\r\n");
            }
            else
            {
            ...
            if (WaitForString(1000) == -1)
            {
                NotificationCenter.DefaultCenter().Notification2Log(0, "Pulsar 1 :Read Right Pulsar Calibration value failed!\r\n");
            }
            else
            {
                if (m_lightdata.ParseValue(m_Buffer) == 0)
            ...
            if (WaitForString(1000) == -1)
            {
                NotificationCenter.DefaultCenter().Notification2Log(0, "Pulsar 1 :Read Pulsar Calibration Time failed!\r\n");
            }
            else
            {
                if (m_lightdata.ParseValue(m_Buffer) == 0)
            ...
            if (side == "Left")
            {
                    ...
                    if (str.IndexOf("close the left led") != -1)
                    {
                        if (str.IndexOf("finish") != -1)
                        {
                            NotificationCenter.DefaultCenter().Notification2Log(0,
                               "Pulsar 1 :Close the LEFT LED:" + nindex.ToString() + "  " + " Success" + "\r\n");
                            return 0;
                        }
                        else
                        {
                            NotificationCenter.DefaultCenter().Notification2Log(0,
                                "Pulsar 1 :Close the LEFT LED:" + nindex.ToString() + "  " + " Faild" + "\r\n");
                            return -1;
                        }
                    }
                }
                else
                {
                    NotificationCenter.DefaultCenter().Notification2Statue(0,
                        "Pulsar 1 :Close the LEFT LED:" + nindex.ToString() + " TimeOut!");
                    return -1;
                }
        ...
        public int Write_LEDData(string side, int nindex, int nvalue)
        {
                ...
                if (side == "Left")
                {
                    ...
                    if (WaitForString(200) != -1)
                    {
                        ...
                        if (str.IndexOf("open left led") != -1)
                        {
                            ...
                            {
                                NotificationCenter.DefaultCenter().Notification2Log(0,
                                "Pulsar 1 :OPEN LEFT LED:" + nindex.ToString() + "  " + nvalue.ToString() + " Success" + "\r\n");
                                return 0;
                            }
                            else
                            {

                                NotificationCenter.DefaultCenter().Notification2Log(0, "Pulsar 1 :OPEN LEFT LED:"
                                    + nindex.ToString() + "  " + nvalue.ToString() + " Fail" + "\r\n");
                                return -1;
                            }
                        }
                    }
                    ...
                    {
                        NotificationCenter.DefaultCenter().Notification2Statue(0,
                        "Pulsar 1 :OPEN LEFT LED:" + nindex.ToString() + " TimeOut!");
                    }
                }
                else
                {
            ...
            {
                NotificationCenter.DefaultCenter().Notification2Statue(0,"ALSC1写入失败" + e.Message);
        ...
        public void CheckAndConnect()
        {
            ...
            {
                NotificationCenter.DefaultCenter().Notification2Statue(0, "ALSC-1打开失败" + ex.Message + "(E0002)");
            }
        }
    }
}
