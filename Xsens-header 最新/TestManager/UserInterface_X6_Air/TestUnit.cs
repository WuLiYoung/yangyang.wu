...
namespace UserInterface
{
    public partial class TestUnit : WeifenLuo.WinFormsUI.Docking.DockContent
    {
       ...
       private void SetMLBSNBufferContext(string str)
       {
           if (str != null)
           {
               tc.m_dicContext[tmMarcos.kContextMLBSNBuffer] = str;
           }
       }
       ...
       private void tbUnitSN_KeyPress(object sender, KeyPressEventArgs e)
       {
           ...
           {
               if (GT_UserInterface.m_TestEngine.IsTesting(0) == 1)//正在测试
               {
                   SetMLBSNBufferContext(tbUnitSN.Text);
               ...
               if (count == 1)
               {
                   ...
                   for (int i = 0; i < 6; i++)
                   {
                      ...
                      // (this.MdiParent as MainForm).m_frmState.SetSN(sn, i);
                       if (GT_UserInterface.m_TestEngine.IsTesting(0) == 0)//等待测试
                       {
                           context.m_dicContext[tmMarcos.kContextMLBSNBuffer] = "";
                       ...
                       else
                       {
                           ...
                           context.m_dicContext[tmMarcos.kContextMLBSNBuffer] = sn;
                     //      (this.MdiParent as MainForm).m_frmState.SetSN(sn, i);
                       }
                       //context.m_dicContext[tmMarcos.kContextMLBSN] = sn;
                       //(this.MdiParent as MainForm).m_frmState.SetSN(sn, i);
