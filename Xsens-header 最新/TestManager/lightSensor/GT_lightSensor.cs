...
{
    public class GT_lightSensor : IModule
    {
        static public Engine m_TestEngine;
        public static lightSensor  m_object;
        ...
        public GT_lightSensor()
        {
            m_object = new lightSensor();
        }
        ...
        public int RegisterModule(object sender, object arg)
        {
            for (int i = 0; i < m_TestEngine.GetEngineCore(null); i++)
            {
                ...
                l["SensorL"] = m_object;
                se.DoString("senl= require \"lightSensors\"");
            }
            return 0;
        }
        ...
        int OnUiLoadFinish(Notification nf)
        {
            ...
            instr.DropDownItems.Add("Sensor 1 Config", null, new EventHandler(OnConfig));
            //这个时候配置已经读取了
            m_object.Init();
            ConnectCom();
            return 0;
